package org.rest.com.RestDemo1.service;

import java.util.ArrayList;
import java.util.Calendar;
import java.util.List;
import java.util.Map;

import org.rest.com.RestDemo1.database.DatabaseClass;
import org.rest.com.RestDemo1.exception.DataNotFoundException;
import org.rest.com.RestDemo1.model.Message;

public class MessageService {
	
	private Map<Long,Message> messages = DatabaseClass.getMessages();
	
	public MessageService() {
		messages.put(1L,new Message(1,"hellowolrd","kishore"));
		messages.put(2L,new Message(2,"hello Jersey","koushik"));
	}

	public List<Message> getAllMessages(){
		/*Message m1 = new Message(1L,"hellowolrd","kishore");Message m2= new Message(2L,"hello Jersey","koushik");
		List<Message> list  = new ArrayList<>(); list.add(m1); list.add(m2); return list;*/	
		return new ArrayList<Message>(messages.values());
	}
	
	// filtering episode:22(java brains)
	public List<Message> getAllMessagesForYear(int year){
		List<Message> messageForYear = new ArrayList<>();
		Calendar cal = Calendar.getInstance();
		for (Message message :messages.values()){
			cal.setTime(message.getCreated());
			if(cal.get(Calendar.YEAR)==year){
				messageForYear.add(message);
			}
		}
		return messageForYear;		
	}
	//pagination episode:22(java brains)
	public List<Message> getAllMessagePaginated(int start, int size){
		ArrayList<Message> list = new ArrayList<Message>(messages.values());
		if(start+size> list.size()){
			return new ArrayList<Message>();
		}
		return list.subList(start, start+size);
	}
	
	//we can direclty return the messsges by their id using the below method
	/*
	public Message getMessage(long id){
		return messages.get(id);
	}*/
	
	//above method modified as below in the episode27:handling exceptions
	
	public Message getMessage(long id){
		Message message = messages.get(id);
		if(message==null){
			throw new DataNotFoundException("Message with id:" + id + "not found");
		}
		return message;
	}
	
	
	public Message addMessage(Message message){
		message.setId(messages.size()+1);
		messages.put(message.getId(),message);
		return message;
	}
	public Message updateMessage(Message message){
		if(message.getId()<=0){
			return null;
		}
		messages.put(message.getId(), message);
		return message;
	}
	public Message removeMessage(long id){
		return messages.remove(id);
	}
	

}
